...
for (cd in cd2){
  ...
  ## SNP QC
  ## ------------------------------
  bm.maf <- select.snps(bm, condition = maf >= 0.05)
  bm.maf@ped$pheno <- y[bm@ped$id]

  bm.NonNA <- select.inds(bm.maf, condition = !is.na(pheno))
  bm.NonNA.maf <- select.snps(bm.NonNA, condition = maf >= 0.05)
  ...
  ## FDR 0.05 で有意な SNP
  q_c <- 0.05
  sig_snps_fdr <- result$id[!is.na(result$q) & result$q < q_c]
  condition_snps_list[[cd]] <- sig_snps_fdr
  ...
  ## chr列名が環境で違うことがあるので吸収（chr / chrom）
  if (!is.null(result$chr)) {
    chr <- result$chr
  } else if (!is.null(result$chrom)) {
    chr <- result$chrom
  } else {
    stop("Cannot find chromosome column in result (expected result$chr or result$chrom).")
  }

  ## chrごとに2色（交互）
  chr_levels <- unique(chr)
  chr_col_map <- rep(c("gray30", "gray70"), length.out = length(chr_levels))
  names(chr_col_map) <- as.character(chr_levels)

  cols <- chr_col_map[as.character(chr)]

  ## FDRで有意なSNPだけ赤に上書き
  is_fdr <- !is.na(result$q) & result$q < q_c
  cols[is_fdr] <- "red"
  ...
  # 固定 p 値カット
  p_c <- 1e-5
  abline(h = -log10(p_c), col = "darkgray", lty = 3)
...
for (cd in cd2){
  ...
  cols <- rep("black", length(result$p))
  cols[is_fdr] <- "red"
...
imp_common_cols <- list(Drought=
  colnames(readRDS(here("out", "imp_RF3_d.RDS")))[colSums(readRDS(here("out", "imp_RF3_d.RDS"))) > 0.01],
  Control= colnames(readRDS(here("out", "imp_RF3_c.RDS")))[colSums(readRDS(here("out", "imp_RF3_c.RDS"))) > 0.01]
)
