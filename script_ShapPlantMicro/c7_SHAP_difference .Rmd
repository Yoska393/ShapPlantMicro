...
for (s in seed_list) {
	...
	imr_drought <- log10(readRDS(here("out", "imr_d5")) + 1) / 2
	imr_control <- log10(readRDS(here("out", "imr_c5")) + 1) / 2
	...
	g <- ggraph(g_diff, layout = "fr") +
		...
		scale_edge_width(name = "Strength", range = c(1, 4), labels = scales::scientific) +
