...
for (s in seed_list) {
	...
	point_size_values <- c("1-2" = 2, "3-5" = 3, "6-10" = 5, "10+" = 8)
	
	# label size
	V(g_diff)$label <- sapply(V(g_diff)$name, get_label)
	V(g_diff)$is_target <- V(g_diff)$name %in% target_nodes
	V(g_diff)$text_size <- ifelse(V(g_diff)$is_target, 3.5, 3)
	...
	g <- ggraph(g_diff, layout = "fr") +
		...
		scale_edge_color_manual(values = c("Drought" = "#D7263D", "Control" = "blue")) +
