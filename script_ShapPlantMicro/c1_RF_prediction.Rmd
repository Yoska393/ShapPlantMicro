...
#write.csv(miname_df, file = "miname.csv", row.names = FALSE)

#metname_df <- data.frame(metname = metname)
#write.csv(metname_df, file = "metname.csv", row.names = FALSE)
...
g <- ggplot(df, aes(x = LF1, y = LF2, color = Type)) +
  ...
  scale_color_manual(values = c("#FF6F61", "orange", "#66B032", "#9B4F96"))
...
imp.gm <- imp.gm[, colSums(imp.gm) >= 0]
