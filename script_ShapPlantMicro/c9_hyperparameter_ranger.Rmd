...
# --- デフォルトパラメータ強調用 ---
default_param <- "mtry=5, node=5, trees=100"
...
df_combined <- bind_rows(summary_list$W4, summary_list$W1)

# === Calculate Average across W4 & W1 ===
summary_avg <- df_combined %>%
  group_by(mtry, min.node.size, num.trees) %>%
  summarise(
    mse.mean = mean(mse.mean),
    mse.sd = sqrt((sum(mse.sd^2)) / 4),  # pooled SD (assuming 2 conditions)
    condition = "Average",
    .groups = "drop"
  )
...
# === Order param_id by Average MSE (ascending) ===
avg_order <- df_all %>%
...
# Optional: highlight default parameter
default_label <- "mtry=5, node=5, trees=500"
df_all <- df_all %>%
  mutate(label_display = ifelse(param_id == default_label, paste0("*", param_id), as.character(param_id)))
...
default_label = "mtry=6, node=5, trees=500"
