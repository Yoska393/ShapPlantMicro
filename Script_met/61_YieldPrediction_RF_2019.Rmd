...
```{r}
ZETAlist <- list(KZ8,KZ11,KZ1_2_8,KZ1_2_11)
names(ZETAlist) <- c("KZ8","KZ11","KZ1_2_8","KZ1_2_11")
names<- names(ZETAlist)
```

```{R}
ZETAlist <- list(KZ8)
names(ZETAlist) <- c("KZ8")
names<- names(ZETAlist)
```

```{R}
ZETAlist <- list(KZ3)
names(ZETAlist) <- c("KZ3")
names<- names(ZETAlist)
```
...
res<-getBLUP(ZETAlist = ZETAlist, y = y)
...
csv
```{R,eval=TRUE}
write.csv(res$resultEachModel, here("out",paste0("2019_blup",res$p,"_OneOut_",CD,"opt.csv" )))
write.csv(res$resultEachModel_all, here("out",paste0("2019_blup_ResAll_",res$p,"_OneOut_",CD,"opt.csv")))
```
...
pred.all<- data.frame(pred.all) %>% select(Name_list)
df.all <- NULL
