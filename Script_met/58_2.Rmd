...
X.list[[1]] <- genome.cd
X.list[[2]] <- com19.cd
X.list[[3]] <- cbind(genome.cd, com19.cd)
names(X.list) <- c("G",
                "M",
                "GM")
...
imp.m <- imp.norm[, (ncol(genome.cd)+1):ncol(imp.norm)]
...
```{r}
pdf(here("out", "58_importancesnp.pdf"), width = 20, height = 20)
print(g)
dev.off()
```
