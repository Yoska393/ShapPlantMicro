...
for(i in 1:ncol(accu)) {
  hist(accu[,i], main = paste(toupper(colnames(accu)[i]), "model"),
       breaks = seq(-1.0, 1.0, 0.1),xlab="COR",ylim = c(0,90))
...
```{r}
#saveRDS(pred, here("out", paste0("19f19_predMet_RF_OneOut", "_W4snp", ".RDS")))
```
...
for(i in 1:ncol(accu)) {
  hist(accu[,i], main = paste(toupper(colnames(accu)[i]), "model"),
       breaks = seq(-1.0, 1.0, 0.1),xlab="COR",ylim = c(0,120))
  abline(v = median(accu[,i]),                       # Add line for mean
       col = "green3",
       lwd = 3)  
  legend(x = "topright", # location of legend within plot area
