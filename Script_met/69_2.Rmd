...
# mic <- scale(mic[, apply(mic, 2, sd) > 1])
# 
# table(apply(mic, 2, sd) > 0)
...
X_set <- list(gen3, grm, met, mic, mir, cbind(genome,mic), cbind(genome,mir))                 # X options
...
# Define X and Y sets with descriptive names
X_set <- list(gen3, grm, met, mic, mir, cbind(gen3,mic), cbind(genome,mir))                 # X options
X_names <- c("genome","grm","metabolome", "micro_c", "micro_r","genome micro_c","genome micro_r")  # Descriptive X names
Y_set <- list(pheno,met)       # Y options
Y_names <- c("phenotype","metabolome")  # Descriptive Y names
...
for (k in 1:length(ni) ){
    ...
    #for (rn in 1:2){
        ...
        saveRDS(PY,here("out","Soy",cd,paste0("Pred_X=", X_names[i], "Y=", Y_names[j],cd,"gm2019_LOO_c_5k.RDS")))
        Y <- PY$Y
...
saveRDS(cmat,here("out","Soy",cd,"cor_gm2019_LOO_c_5k.RDS"))
print(cmat)
```
...
```{R,fig.dim = c(8, 4)}
#c -> c_5k
pred.all<-readRDS(here("out","Soy",cd,"cor_gm2019_LOO_c.RDS"))
...
rda<-readRDS(here("out","Soy",cd,"cor_gm2019_LOO_c.RDS"))
rda5<-rda[4,]
...
#microcountres<-microcount$resultEachModel
       "yPredYield_BLUP_micro_count_withRFpredRDApredW1com.RDS"
pred_count<-readRDS(here("out",paste0("yPredYield_BLUP_micro_count_withRFpredRDApred", CD, "com.RDS")))
gmicrocountres<-pred_count$resultEachModel[2,]
predcountres<-pred_count$resultEachModel[3,]
# 75
