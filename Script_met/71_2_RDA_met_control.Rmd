...
#for (k in 1 ){
    ...
    #for (rn in 1){
        ...
        plan(multisession)
        #tic()
        cv_result <- rrda.cv(X = X_train, Y = Y_train,lambda = Lambda,maxrank = 30,verbose = F)
        #toc()
        plan(sequential)
        #sink()
        ...
        saveRDS(PY,here("out","Soy",cd,paste0("Pred_X=", X_names[i], "Y=", Y_names[j],cd,"gm2019_LOO_meta_3k.RDS")))
        Y <- PY$Y
...
saveRDS(cmat,here("out","Soy",cd,"cor_gm2019_LOO_meta_3k.RDS"))
```
