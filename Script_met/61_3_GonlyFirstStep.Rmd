...
if (CD == "W4"){
  ...
  predmet19RF<-RF[["G"]]#change to G only
  
  RF2 <- readRDS(here("out", "19f20_predMet_RF_OneOut_W4.rds"))
  predmet19f20RF<-RF2[["G"]] #change to G only
...
ZETAlist <- list(KZ1,KZ8,KZ9,KZ11,KZ12,KZ3,KZ1_8,KZ1_9,KZ1_11,KZ1_12,KZ1_3)
names(ZETAlist) <- c("KZ1","KZ8","KZ9","KZ11","KZ12","KZ3","KZ1_8","KZ1_9","KZ1_11","KZ1_12","KZ1_3")
...
csv
```{R,eval=TRUE}
write.csv(resultEachModel, here("out",paste0("2019_blup",p,"_OneOut_",CD,"fG.csv" )))
write.csv(resultEachModel_all, here("out",paste0("2019_blup_ResAll_",p,"_OneOut_",CD,"fG.csv")))
```
...
#p<- c("KZ1KZ8KZ9KZ11KZ12KZ3KZ1_8KZ1_9KZ1_11KZ1_12KZ1_3")
#pred.all<-resultEachModel
pred.all <- read.csv(here("out",paste0("2019_blup",p,"_OneOut_",CD,"fG.csv")),row.names=1)

model.name <- c("G","PredMetBlupf19","PredMetBlupf20","PredMetRFf19","PredMetRFf20","TrueMet","G+PredMetBLUPf19","G+PredMetBLUPf20","G+PredMetRF_f19","G+PredMetRF_f20","G+TrueMet")
trait.name <- colnames(pred.all)
df.all <- NULL

#trait order
#Name_list <- names(pred.all) %>% sort()
#pred.all<- pred.all %>% select(all_of(Name_list))
...
```{R,fig.dim = c(8, 4)}
#p<- c("KZ1KZ8KZ9KZ11KZ12KZ3KZ1_8KZ1_9KZ1_11KZ1_12KZ1_3")
#("G","PredMetBlupf19","PredMetBlupf20","PredMetRFf19","PredMetRFf20","TrueMet","G+PredMetBLUPf19","G+PredMetBLUPf20","G+PredMetRF_f19","G+PredMetRF_f20","G+TrueMet")
#pred.all<-resultEachModel

pred.all <- read.csv(here("out",paste0("2019_blup",p,"_OneOut_",CD,"fG.csv")),row.names=1)
pred.all<- pred.all[c(1,7,8,9,10,11),]
model.name <- c("G","G+PredMetBLUPf19","G+PredMetBLUPf20","G+PredMetRF_f19","G+PredMetRF_f20","G+TrueMet")
